//! Hear the time-stretch effect, no GUI required.
//!
//! Synthesizes a short copyright-free music clip (a plucked arpeggio over a
//! bass line — generated right here, so there is nothing to license), writes
//! it to a temp WAV, plays it through the default output device, and steps
//! the tempo live:
//!
//! - 3 s at normal speed
//! - 2 s at 75%
...
//! - 2 s at 150%
//!
//! Pitch stays put the whole way — only the tempo moves. Note the engine
//! buffers roughly half a second of audio ahead of the device, so each
//! change is *heard* about that long after it is printed.
//!
//! Run with: `cargo run --example time_stretch --features stretch`
...
fn main() {
    let path = std::env::temp_dir().join("audio-stack-rs-time-stretch-demo.wav");
    ...
    // (wall seconds to hold, tempo ratio, label)
    let scenario = [
        (3.0, 1.0, "normal speed"),
        (2.0, 0.75, "75% speed"),
        (2.0, 0.5, "50% speed"),
        (1.0, 1.0, "100% speed again"),
        (2.0, 1.5, "150% speed"),
    ];

    for (hold, ratio, label) in scenario {
        println!("\n▶ {label} (ratio {ratio}) for {hold} s");
        engine.set_time_stretch(true, ratio);
...
/// Eight bars of an Am–F–C–G arpeggio over a bass line at 110 BPM: enough
/// musical texture (attacks, sustained bass, a moving line) to hear both the
/// tempo change and the pitch *not* changing.
fn compose() -> Vec<f32> {
    let beat = 60.0 / 110.0;
    let bar = beat * 4.0;
    let total = (8.0 * bar * SAMPLE_RATE as f64) as usize;
    let mut left = vec![0.0f32; total];
    let mut right = vec![0.0f32; total];

    // Chords as semitone offsets from A4 = 440 Hz: (bass, [arpeggio notes]).
    ...
    let pattern = [0usize, 1, 2, 3, 2, 3, 1, 2]; // eighth-note arpeggio

    for bar_index in 0..8 {
        let (bass, notes) = chords[bar_index % chords.len()];
        let bar_start = bar_index as f64 * bar;

        // Bass: one plucked note per half bar, dead center.
        for half in 0..2 {
        ...
        // Arpeggio: eighth notes, leaning right, with a quiet detuned double
        // leaning left for width.
...
/// Adds one exponentially decaying tone (a cheap pluck: fundamental plus a
/// touch of second harmonic) into a mono channel.
...
fn wav_bytes(interleaved: &[f32]) -> Vec<u8> {
    ...
    bytes.extend_from_slice(&(SAMPLE_RATE * 2 * 2).to_le_bytes()); // byte rate
...
/// Prints the playhead and the time-stretch echoes — the "UI" of this demo.
struct PrintSink;
impl EventSink for PrintSink {
    fn send_event(&self, event: EngineEvent) {
        match event {
            ...
            EngineEvent::TimeStretch { enabled, ratio } => {
                println!("\r  engine confirms: enabled={enabled} ratio={ratio}");
            }
