...
/// how a poller for a station the listener has left stops on its own. Spawned
/// on the engine's owned network runtime.
pub fn spawn(
...
mod tests {
    ...
    fn every_provider_answers() {
        ...
        for source in sources {
            let (update, wait) = runtime
                .block_on(fetch(&client, &source))
